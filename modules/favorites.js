...
(function() {
    ...
    window.ScalePlusFavorites = {
        ...
        init() {
            ...
            this.setupClearFiltersObserver();
                ...
                clearBtn.addEventListener('click', (e) => {
                    ...
                    setTimeout(() => {
                        ...
                        if (formId && this.isEnabled()) {
                            ...
                            if (defaultFilter && !hasPendingFilter && !this.hasProcessedPendingFilter) {
                                console.log('[ScalePlus Favorites] Applying default filter after clear:', defaultFilter);
                                this.fetchSavedFilter(defaultFilter)
                                    .then(savedFilters => {
                                        this.applySavedFilters(savedFilters);
                                    })
                                    .catch(err => {
                                        console.warn('[ScalePlus Favorites] Failed to fetch or apply saved filter after clear:', err);
                                    });
                            } else if (hasPendingFilter) {
                                console.log('[ScalePlus Favorites] Skipping default filter application after clear - pending filter exists');
                            } else if (this.hasProcessedPendingFilter) {
                                console.log('[ScalePlus Favorites] Skipping default filter application after clear - pending filter was just processed');
                            }
                        }
